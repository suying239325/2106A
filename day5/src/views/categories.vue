<template>
	<div class="">
		...
		<el-card class="mtop16">
			<el-button type="primary" class="mtop16" @click="addTO">添加分类</el-button>
			<el-table :data="tableData" border row-key="cat_id">
				...
				<el-table-column prop="cat_deleted" label="是否有效" height="80">
					<template slot-scope="scope">
						<div>
							<i class="el-icon-success el-icon-skyblue" v-if="scope.row.cat_deleted"></i>
				...
				<el-table-column prop="cat_level" label="排序" height="80">
					<template slot-scope="scope">
						<div>
							<el-tag :type="Tag[scope.row.cat_level]">{{
                scope.row.cat_level | filter
              }}</el-tag>
						</div>
					</template>
				</el-table-column>
				<el-table-column prop="goods_id" label="操作" height="80">
					<template slot-scope="scope">
						<div>
							<el-button size="mini" type="primary" icon="el-icon-edit" @click="revise(scope.row)">
							</el-button>
							<el-button size="mini" type="danger" icon="el-icon-delete"
								@click="remove(scope.row.cat_id)"></el-button>
						</div>
					</template>
				</el-table-column>
			</el-table>
		...
		<!-- 修改商品分类 -->
		<el-dialog title="修改商品分类" :visible.sync="modifyClassification" width="30%">
